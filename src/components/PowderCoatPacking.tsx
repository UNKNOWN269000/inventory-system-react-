...
export default function PowderCoatPacking() {
  ...
  const calcPremiumTotal = () => {
    ...
    const bundles = parseInt(form.premiumTotalBundles) || 0;
    return (one * bundles).toString();
  };

  const calcNonBrandTotal = () => {
    const one = parseInt(form.nonBrandOneQty) || 0;
    const bundles = parseInt(form.nonBrandTotalBundles) || 0;
    return (one * bundles).toString();
  };
  ...
  const buildRecord = () => ({
    ...
    colour: form.colour,
    re_powder_qty: form.rePowderQty,
    ...
    premium_total_bundles: premiumEnabled ? form.premiumTotalBundles : null,
    ...
    nonbrand_total_bundles: nonBrandEnabled ? form.nonBrandTotalBundles : null,
    ...
    weightbar_one_qty: weightBarEnabled ? form.weightBarOneQty : null,
  ...
  const editPendingRecord = (index: number) => {
    ...
    setForm({
      ...
      productionType: record.production_type,
      profile: record.profile,
      colour: record.colour,
      rePowderQty: record.re_powder_qty || "",
      premiumPackNo: record.premium_pack_no || "",
      premiumOneQty: record.premium_one_qty || "",
      premiumTotalBundles: record.premium_total_bundles || "",
      ...
      nonBrandTotalBundles: record.nonbrand_total_bundles || "",
      ...
      weightBarOneQty: record.weightbar_one_qty || "",
  ...
  const glassInput = "w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-zinc-100 outline-none focus:border-green-500 backdrop-blur-sm transition-colors";
  const glassBtnPrimary = "rounded-lg bg-gradient-to-r from-green-500 to-emerald-500 px-4 py-2.5 text-sm font-bold text-black shadow-lg shadow-green-500/20 transition hover:shadow-green-500/40 disabled:opacity-50";
    ...
    <div className="min-h-screen text-zinc-100">
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <Link href="/home" className="flex items-center gap-3">
              <div className="grid h-9 w-9 place-items-center rounded-lg bg-gradient-to-br from-green-500 to-emerald-500 font-bold text-black shadow-lg shadow-green-500/20">U</div>
              <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={logout} className="rounded-md border border-zinc-800 px-3 py-1.5 text-sm text-zinc-300 transition hover:border-green-400 hover:text-green-300">Logout</button>
          </div>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-7xl px-6 py-8">
        <nav className="flex flex-wrap items-center gap-1 text-xs text-zinc-500">
          ...
          <span className="text-zinc-700">/</span>
          <span className="text-green-300">Powder Coat</span>
        </nav>
        ...
        <div className={`mt-4 overflow-hidden p-8 ${glassCard}`}>
          ...
          <div className="relative flex flex-wrap items-center justify-between gap-4">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-green-500/30 bg-green-500/10 px-3 py-1 text-xs font-medium text-green-400">
                <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-green-400" />
                Packing Module
              </div>
              <h1 className="mt-3 text-3xl font-bold text-white">Powder Coat Packing</h1>
              <p className="text-sm text-zinc-400">Record powder coat packing data</p>
            </div>
        ...
        {pendingRecords.length > 0 && (
          <div className={`mt-6 overflow-hidden ${glassCard}`}>
            <div className="border-b border-white/10 bg-gradient-to-r from-green-500/10 to-emerald-500/10 px-6 py-3 flex items-center justify-between">
                ...
                <tbody>
                  {pendingRecords.map((record, index) => (
                    <tr key={record.id} className="border-b border-white/5 transition hover:bg-white/5">
                      <td className="px-3 py-3 font-bold text-green-400">{record.bucket_no}</td>
                      <td className="px-3 py-3 text-zinc-300">{record.profile}</td>
                      <td className="px-3 py-3 text-zinc-300">{record.production_type}</td>
                      <td className="px-3 py-3 text-zinc-300">{record.colour}</td>
                      <td className="px-3 py-3 text-zinc-300">{record.length}m</td>
                      <td className="px-3 py-3">{record.premium_enabled ? <span className="rounded-full bg-green-500/20 px-2 py-0.5 text-xs text-green-300">Yes</span> : <span className="text-zinc-600">—</span>}</td>
            ...
            <div className="border-t border-white/10 bg-gradient-to-r from-green-500/10 to-emerald-500/10 px-6 py-4 flex items-center justify-between">
        ...
        <div className={`mt-6 overflow-hidden ${glassCard}`}>
          <div className="border-b border-white/10 bg-gradient-to-r from-green-500/10 to-emerald-500/10 px-6 py-3 flex items-center justify-between">
            ...
            <button onClick={loadRecentData} className="rounded-md border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-zinc-300 backdrop-blur-sm transition hover:border-green-400 hover:text-green-300">
              ↻ Refresh
            </button>
          </div>
          {recentData.length === 0 ? (
                ...
                <tbody>
                  {recentData.map((row) => (
                    <tr key={row.id} className="border-b border-white/5 transition hover:bg-white/5">
                      <td className="px-3 py-3 font-bold text-green-400">{row.bucket_no}</td>
                      ...
                      <td className="px-3 py-3">{row.premium_enabled ? <span className="rounded-full bg-green-500/20 px-2 py-0.5 text-xs text-green-300">Yes</span> : <span className="text-zinc-600">—</span>}</td>
          ...
          <div className="fixed inset-0 z-50 overflow-y-auto">
            <div className="flex min-h-full items-center justify-center p-4">
              <div className={`w-full max-w-2xl overflow-hidden my-8 ${glassCard}`}>
                <div className="sticky top-0 z-10 border-b border-white/10 bg-gradient-to-r from-green-500/10 to-emerald-500/10 px-6 py-4 backdrop-blur-xl">
                  <div className="flex items-center justify-between">
                    <h3 className="flex items-center gap-2 text-xl font-bold text-white">
                      ...
                      {editingRecentId ? "Edit Packing" : "Add Packing"}
                    </h3>
                ...
                <form className="max-h-[80vh] overflow-y-auto p-6 space-y-4">
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                      ...
                      <input type="date" value={form.productionDate} onChange={(e) => setForm({ ...form, productionDate: e.target.value })} required className={glassInput} />
                    </div>
                    <div>
                  ...
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                      ...
                      <input type="number" value={form.bucketNo} onChange={(e) => setForm({ ...form, bucketNo: e.target.value })} placeholder="000" required className={glassInput} />
                    </div>
                    <div>
                  ...
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                      ...
                      <select value={form.productionType} onChange={(e) => setForm({ ...form, productionType: e.target.value })} required className={glassInput}>
                          ...
                          {filteredProfiles.length === 0 ? (
                            ...
                            filteredProfiles.map((p) => (
                              <div
                                ...
                                className="cursor-pointer border-b border-zinc-900 px-3 py-2 text-sm text-zinc-300 hover:bg-green-500/20 hover:text-green-300"
                              >
                                {p}
                              </div>
                            ))
                  ...
                  <div>
                    ...
                    <div className="mb-3">
                      <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-white/10 bg-white/5 p-3 hover:border-green-500/30">
                        <input type="checkbox" checked={premiumEnabled} onChange={(e) => setPremiumEnabled(e.target.checked)} className="h-5 w-5 rounded border-white/10 bg-white/5 accent-green-500" />
                        <span className="text-sm font-bold uppercase text-zinc-400">Premium Full Pack</span>
                      </label>
                      {premiumEnabled && (
                        <div className="mt-2 space-y-3 rounded-2xl border border-green-500/20 bg-green-500/5 p-4">
                          <div>
                            <label className="mb-1 block text-[10px] font-medium uppercase tracking-wider text-zinc-400">Pack No</label>
                            <input type="text" value={form.premiumPackNo} onChange={(e) => setForm({ ...form, premiumPackNo: e.target.value })} className={`${glassInput} py-2`} />
                          </div>
                          <div className="grid grid-cols-2 gap-3">
                            ...
                            <div>
                              ...
                              <input type="number" value={form.premiumTotalBundles} onChange={(e) => setForm({ ...form, premiumTotalBundles: e.target.value })} className={`${glassInput} py-2`} />
                            </div>
                          </div>
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                          ...
                          <label className="flex cursor-pointer items-center gap-2 border-t border-green-500/10 pt-2">
                          ...
                          {premiumPcsEnabled && (
                            <div className="space-y-2 rounded-xl border border-white/5 bg-black/20 p-3">
                              <div>
                                ...
                                <input type="text" value={form.premiumPcsPackNo} onChange={(e) => setForm({ ...form, premiumPcsPackNo: e.target.value })} className={`${glassInput} py-1.5 text-xs`} />
                              </div>
                              <div className="grid grid-cols-3 gap-2">
                                <div>
                                  <label className="mb-1 block text-[9px] font-medium uppercase tracking-wider text-zinc-400">Bundle Qty</label>
                                  <input type="text" value={form.premiumPcsOneQty} onChange={(e) => setForm({ ...form, premiumPcsOneQty: e.target.value })} className={`${glassInput} py-1.5 text-xs`} />
                                </div>
                                <div>
                                  <label className="mb-1 block text-[9px] font-medium uppercase tracking-wider text-zinc-400">Total Qty</label>
                                  <input type="text" value={form.premiumPcsTotalQty} onChange={(e) => setForm({ ...form, premiumPcsTotalQty: e.target.value })} className={`${glassInput} py-1.5 text-xs`} />
                                </div>
                                <div>
                    ...
                    <div className="mb-3">
                      <label className="flex cursor-pointer items-center gap-3 rounded-xl border border-white/10 bg-white/5 p-3 hover:border-white/30">
                        <input type="checkbox" checked={nonBrandEnabled} onChange={(e) => setNonBrandEnabled(e.target.checked)} className="h-5 w-5 rounded border-white/10 bg-white/5 accent-green-500" />
                      ...
                      {nonBrandEnabled && (
                        <div className="mt-2 space-y-3 rounded-2xl border border-white/10 bg-white/5 p-4">
                          <div>
                            <label className="mb-1 block text-[10px] font-medium uppercase tracking-wider text-zinc-400">Pack No</label>
                            <input type="text" value={form.nonBrandPackNo} onChange={(e) => setForm({ ...form, nonBrandPackNo: e.target.value })} className={`${glassInput} py-2`} />
                          </div>
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              <label className="mb-1 block text-[10px] font-medium uppercase tracking-wider text-zinc-400">One Bundle Qty</label>
                            ...
                            <div>
                              <label className="mb-1 block text-[10px] font-medium uppercase tracking-wider text-zinc-400">Total Bundles</label>
                              <input type="number" value={form.nonBrandTotalBundles} onChange={(e) => setForm({ ...form, nonBrandTotalBundles: e.target.value })} className={`${glassInput} py-2`} />
                            </div>
                          </div>
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                          ...
                          {nonBrandPcsEnabled && (
                            <div className="space-y-2 rounded-xl border border-white/5 bg-black/20 p-3">
                              <div>
                                <label className="mb-1 block text-[9px] font-medium uppercase tracking-wider text-zinc-400">Pcs Pack No</label>
                                <input type="text" value={form.nonBrandPcsPackNo} onChange={(e) => setForm({ ...form, nonBrandPcsPackNo: e.target.value })} className={`${glassInput} py-1.5 text-xs`} />
                              </div>
                              <div className="grid grid-cols-3 gap-2">
                                <div>
                                  <label className="mb-1 block text-[9px] font-medium uppercase tracking-wider text-zinc-400">Bundle Qty</label>
                                  <input type="text" value={form.nonBrandPcsOneQty} onChange={(e) => setForm({ ...form, nonBrandPcsOneQty: e.target.value })} className={`${glassInput} py-1.5 text-xs`} />
                                </div>
                                <div>
                                  <label className="mb-1 block text-[9px] font-medium uppercase tracking-wider text-zinc-400">Total Qty</label>
                                  <input type="text" value={form.nonBrandPcsTotalQty} onChange={(e) => setForm({ ...form, nonBrandPcsTotalQty: e.target.value })} className={`${glassInput} py-1.5 text-xs`} />
                                </div>
                                <div>
                                  <label className="mb-1 block text-[9px] font-medium uppercase tracking-wider text-zinc-400">Avg Weight</label>
                    ...
                    <div>
                      ...
                      {weightBarEnabled && (
                        <div className="mt-2 space-y-3 rounded-2xl border border-blue-500/20 bg-blue-500/5 p-4">
                          <div>
                            <label className="mb-1 block text-[10px] font-medium uppercase tracking-wider text-zinc-400">Pack No</label>
                            <input type="text" value={form.weightBarPackNo} onChange={(e) => setForm({ ...form, weightBarPackNo: e.target.value })} className={`${glassInput} py-2`} />
                          </div>
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              <label className="mb-1 block text-[10px] font-medium uppercase tracking-wider text-zinc-400">Bundle Qty</label>
                              <input type="number" value={form.weightBarOneQty} onChange={(e) => setForm({ ...form, weightBarOneQty: e.target.value })} className={`${glassInput} py-2`} />
                            </div>
                            <div>
                              <label className="mb-1 block text-[10px] font-medium uppercase tracking-wider text-zinc-400">Avg Weight</label>
                  ...
                  <div>
                    <label className="mb-1 block text-xs font-medium uppercase tracking-wider text-zinc-400">Re-Powder Qty</label>
                    <input type="number" value={form.rePowderQty} onChange={(e) => setForm({ ...form, rePowderQty: e.target.value })} placeholder="0" className={glassInput} />
                  </div>

                  <div className="mt-6 flex gap-3">
                    <button type="button" onClick={closeModal} className="flex-1 rounded-xl border border-white/10 bg-white/5 px-6 py-3.5 text-sm font-bold uppercase text-zinc-300 backdrop-blur-sm transition hover:border-green-400 hover:text-green-300">
                      Cancel
                    </button>
                    {editingRecentId === null ? (
                      <button type="button" onClick={handleAddToPending} disabled={loading} className="flex-1 rounded-xl bg-gradient-to-r from-green-500 to-emerald-500 px-6 py-3.5 text-sm font-black uppercase text-black shadow-lg shadow-green-500/20 transition hover:shadow-green-500/40 disabled:opacity-50">
                        {editingIndex !== null ? "Update Pending" : loading ? "Saving..." : "Add to Pending"}
                      </button>
                    ) : (
                      <button type="button" onClick={handleUpdateRecent} disabled={loading} className="flex-1 rounded-xl bg-gradient-to-r from-green-500 to-emerald-500 px-6 py-3.5 text-sm font-black uppercase text-black shadow-lg shadow-green-500/20 transition hover:shadow-green-500/40 disabled:opacity-50">
