...
export default function BucketDetails() {
  ...
  const filteredBuckets = buckets.filter((bucket) => {
    const matchesSearch = searchTerm === "" || 
      ...
      bucket.profile.toLowerCase().includes(searchTerm.toLowerCase()) ||
      bucket.type.toLowerCase().includes(searchTerm.toLowerCase()) ||
      bucket.shift.toLowerCase().includes(searchTerm.toLowerCase()) ||
      bucket.batch_no.toLowerCase().includes(searchTerm.toLowerCase());
      ...
      <div className="mx-auto max-w-7xl px-6 py-8">
        ...
        <div className={`mt-4 overflow-hidden p-8 ${glassCard}`}>
          ...
          <div className="relative flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <h1 className="mt-3 text-3xl font-bold text-white">Bucket Details</h1>
              <p className="text-sm text-zinc-400">Complete bucket lifecycle and traceability</p>
            </div>
            <button onClick={loadBuckets} disabled={loading} className="rounded-md border border-white/10 bg-white/5 px-4 py-2 text-sm text-zinc-300 backdrop-blur-sm transition hover:border-pink-400 hover:text-pink-300 disabled:opacity-50">
        ...
        <div className={`mt-6 p-6 ${glassCard}`}>
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="relative">
              ...
              />
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-500">
                <circle cx="11" cy="11" r="8" />
                <path d="M21 21l-4.35-4.35" />
              </svg>
              {searchTerm && (
                <button onClick={() => setSearchTerm("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-zinc-500 transition hover:text-zinc-300">
            ...
            <div className="relative">
              <input type="date" value={dateFilter} onChange={(e) => setDateFilter(e.target.value)} className={glassInput} />
              {dateFilter && (
                <button onClick={() => setDateFilter("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-pink-400 transition hover:text-pink-300">
                  Clear
                </button>
              )}
            </div>
          ...
          <div className="mt-3 flex items-center justify-between text-xs text-zinc-400">
            <span>Showing {filteredBuckets.length} of {buckets.length} records</span>
            {(searchTerm || dateFilter) && (
              <button onClick={() => { setSearchTerm(""); setDateFilter(""); }} className="text-pink-400 transition hover:text-pink-300">
                Clear all filters
              </button>
            )}
          </div>
        ...
        <div className={`mt-6 overflow-hidden ${glassCard}`}>
          <div className="border-b border-white/10 bg-gradient-to-r from-pink-500/10 to-purple-500/10 px-6 py-3">
            <h2 className="text-lg font-semibold text-white">All Bucket Records</h2>
          </div>
          {loading ? (
            <div className="p-8 text-center text-zinc-500">Loading bucket records…</div>
          ) : filteredBuckets.length === 0 && searchTerm ? (
            ...
            <div className="p-8 text-center text-zinc-500">No bucket records found in the database.</div>
          ) : (
                ...
                <tbody>
                  {filteredBuckets.map((row) => (
                    <tr key={row.id} className="border-b border-white/5 transition hover:bg-white/5">
                      ...
                      <td className="px-3 py-3 text-zinc-300">{row.ext_date}</td>
                      <td className="px-3 py-3 text-zinc-300">{row.shift || "—"}</td>
                      <td className="px-3 py-3 text-zinc-300">{row.type || "—"}</td>
                      <td className="px-3 py-3 text-zinc-300">{row.profile || "—"}</td>
                      <td className="px-3 py-3 text-zinc-300">{row.length || "—"}</td>
                      <td className="px-3 py-3 text-zinc-300">{row.qty || "—"}</td>
                      <td className="px-3 py-3 text-zinc-300">
                        {row.aging_date ? (
                          <span className="rounded-full bg-purple-500/20 px-2 py-0.5 text-xs text-purple-300">{row.aging_date}</span>
                        ) : (
                          <span className="text-zinc-600">—</span>
                        )}
                      </td>
                      <td className="px-3 py-3 text-zinc-300">{row.in_time || "—"}</td>
                      <td className="px-3 py-3 text-zinc-300">{row.out_time || "—"}</td>
                      <td className="px-3 py-3">
                        {row.hardness !== null && row.hardness !== 0 ? (
                          <span className="rounded-full bg-pink-500/20 px-2 py-0.5 text-xs font-bold text-pink-300">{row.hardness}</span>
                        ) : (
                          <span className="text-zinc-600">—</span>
                        )}
                      </td>
