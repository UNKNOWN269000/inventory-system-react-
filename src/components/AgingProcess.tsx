...
export default function AgingProcess() {
  ...
  const [currentBatch, setCurrentBatch] = useState<{ date: string; buckets: SelectedBucket[]; inTime: string; outTime: string } | null>(null);
  ...
  const loadRecentData = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from("aging_process")
        .select("id, aging_date, total_weight, batch_in_time, batch_out_time, created_at")
  ...
  const loadAvailableBuckets = async () => {
    try {
      // Only show buckets where aging_date, in_time, out_time are NULL
      // Don't filter by submitted - just check aging fields are empty
      const { data, error } = await supabase
        .from("bucket_income")
  ...
  const addBucket = (bucket: AvailableBucket) => {
    setSelectedBuckets([...selectedBuckets, { id: bucket.id, bucket_no: bucket.bucket_no, weight: bucket.total_weight }]);
    setBucketSearch("");
  ...
  const totalWeight = selectedBuckets.reduce((sum, b) => sum + parseFloat(b.weight || "0"), 0).toFixed(2);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setCurrentBatch({ date: agingDate, buckets: selectedBuckets, inTime: batchInTime, outTime: batchOutTime });
    setShowAddModal(false);
  };
  ...
  const finalizeSubmit = async () => {
    ...
    // Validation: require times
    if (!batchInTime || !batchOutTime) {
      alert("Please enter both Batch In Time and Batch Out Time");
      return;
    }

    const finalTotalWeight = currentBatch.buckets.reduce((s, b) => s + parseFloat(b.weight || "0"), 0).toFixed(2);

    setShowCloudSync(true);
    setLoading(true);
    try {
      // Step 1: Update bucket_income table with aging details
      ...
      // Step 2: Save to aging_process table
      const newEntry: any = {
        aging_date: currentBatch.date,
        selected_buckets: currentBatch.buckets.map((b) => ({ bucket_no: b.bucket_no, weight: b.weight })),
      ...
      <div className="mx-auto max-w-7xl px-6 py-8">
        ...
        <div className={`mt-4 overflow-hidden p-8 ${glassCard}`}>
          ...
          <div className="relative flex items-center justify-between">
            ...
            <button onClick={openModal} className={glassBtnPrimary}>+ Add New Aging Record</button>
          </div>
        </div>
        ...
        <div className={`mt-6 p-6 ${glassCard}`}>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-zinc-500">
                  {currentBatch ? `${currentBatch.buckets.length} bucket(s) ready to finalize` : "No batch in progress"}
                </p>
              </div>
            </div>
          </div>
          ...
          {currentBatch ? (
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                <div className="rounded-xl border border-white/10 bg-white/5 p-3 backdrop-blur-sm">
                  <p className="text-xs uppercase tracking-wider text-zinc-500">Aging Date</p>
                  <p className="mt-1 font-semibold text-pink-400">{currentBatch.date}</p>
                </div>
                <div className="rounded-xl border border-white/10 bg-white/5 p-3 backdrop-blur-sm">
                  <p className="text-xs uppercase tracking-wider text-zinc-500">Total Buckets</p>
                  <p className="mt-1 font-semibold text-white">{currentBatch.buckets.length}</p>
                </div>
                <div className="rounded-xl border border-white/10 bg-white/5 p-3 backdrop-blur-sm">
                  <p className="text-xs uppercase tracking-wider text-zinc-500">Total Weight</p>
                  <p className="mt-1 font-bold text-pink-400">{currentBatch.buckets.reduce((s, b) => s + parseFloat(b.weight || "0"), 0).toFixed(2)} kg</p>
                </div>
                <div className="rounded-xl border border-white/10 bg-white/5 p-3 backdrop-blur-sm">
                  <p className="text-xs uppercase tracking-wider text-zinc-500">In / Out Time</p>
                  <p className="mt-1 text-zinc-200">{currentBatch.inTime || "—"} / {currentBatch.outTime || "—"}</p>
                </div>
              </div>
              <div className="rounded-xl border border-white/10 bg-white/5 p-4 backdrop-blur-sm">
                <p className="mb-2 text-xs uppercase tracking-wider text-zinc-500">Selected Buckets</p>
                <div className="flex flex-wrap gap-2">
                  {currentBatch.buckets.map((b) => (
                    <span key={b.bucket_no} className="rounded-md border border-pink-500/30 bg-pink-500/10 px-2 py-1 text-xs text-pink-300">
                      {b.bucket_no} <span className="text-zinc-500">({b.weight} kg)</span>
                    </span>
                  ))}
                </div>
              </div>
            ...
            <div className="py-12 text-center text-zinc-500">No records yet.</div>
          )}
        </div>
        ...
        <div className={`mt-6 p-6 ${glassCard}`}>
          <div className="mb-4 flex items-center justify-between border-b border-white/10 pb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-white">Recent Aging Records</h2>
                <p className="text-xs text-zinc-500">Last 3 days from database</p>
              </div>
            </div>
                ...
                <tbody>
                  {recentData.map((row) => (
                    <tr key={row.id} className="border-b border-white/5 transition hover:bg-white/5">
                      <td className="px-3 py-2 font-semibold text-pink-400">{row.aging_date}</td>
                      <td className="px-3 py-2 font-bold text-pink-400">{row.total_weight}</td>
                      <td className="px-3 py-2 text-zinc-300">{row.batch_in_time || "—"}</td>
                      <td className="px-3 py-2 text-zinc-300">{row.batch_out_time || "—"}</td>
                      <td className="px-3 py-2 text-zinc-500 text-xs">{row.created_at ? new Date(row.created_at).toLocaleString() : "N/A"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="mx-auto max-w-7xl px-6 py-4">
            <div className="mb-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
              <div>
                ...
                <p className="text-lg font-bold text-white">{currentBatch.buckets.length}</p>
              </div>
              <div>
                <p className="text-xs text-zinc-500">Total Batch Weight</p>
                <p className="text-lg font-bold text-pink-400">{currentBatch.buckets.reduce((s, b) => s + parseFloat(b.weight || "0"), 0).toFixed(2)} kg</p>
              </div>
              <div>
                <label className="text-xs text-zinc-500">Batch In Time</label>
                <input type="time" value={batchInTime} onChange={(e) => setBatchInTime(e.target.value)} className="mt-1 w-full rounded-md border border-white/10 bg-black/50 px-2 py-1 text-sm text-zinc-100 outline-none focus:border-pink-500" />
              </div>
              <div>
                <label className="text-xs text-zinc-500">Batch Out Time</label>
                <input type="time" value={batchOutTime} onChange={(e) => setBatchOutTime(e.target.value)} className="mt-1 w-full rounded-md border border-white/10 bg-black/50 px-2 py-1 text-sm text-zinc-100 outline-none focus:border-pink-500" />
              </div>
            </div>
            <button onClick={finalizeSubmit} disabled={loading} className="w-full rounded-lg bg-gradient-to-r from-pink-500 to-purple-600 px-4 py-3 text-sm font-bold text-white shadow-lg shadow-pink-500/20 transition hover:shadow-pink-500/40 disabled:opacity-50">
          ...
          <div className="fixed inset-0 z-50 grid place-items-center p-4">
            <div className={`w-full max-w-2xl overflow-hidden ${glassCard}`}>
              <div className="border-b border-white/10 bg-gradient-to-r from-pink-500/10 to-purple-500/10 px-6 py-4 backdrop-blur-xl">
                <h3 className="text-xl font-semibold text-white">Add Aging Record</h3>
              </div>
              <form onSubmit={handleSave} className="max-h-[80vh] overflow-y-auto p-6 space-y-4">
                <div>
                  <label className="mb-1 block text-xs font-medium uppercase tracking-wider text-zinc-400">Aging Date</label>
                  <input type="date" value={agingDate} onChange={(e) => setAgingDate(e.target.value)} required className={glassInput} />
                </div>

                <div className="relative">
                  <label className="mb-1 block text-xs font-medium uppercase tracking-wider text-zinc-400">Select Buckets</label>
                  <input
                    type="text"
                    value={bucketSearch}
                    onChange={(e) => { setBucketSearch(e.target.value); setShowBucketDropdown(true); }}
                  ...
                  {showBucketDropdown && (
                    <div className="absolute z-10 mt-1 max-h-60 w-full overflow-y-auto rounded-lg border border-zinc-800 bg-zinc-950 shadow-xl">
                      {filteredBuckets.length === 0 ? (
                        <div className="px-3 py-2 text-sm text-zinc-500">No available buckets found</div>
                      ) : (
                        filteredBuckets.map((b) => (
                          ...
                          >
                            <div className="flex items-center justify-between">
                              <span className="font-semibold text-pink-400">{b.bucket_no}</span>
                              <span className="text-xs text-zinc-500">{b.total_weight} kg</span>
                            </div>
                            <div className="text-xs text-zinc-500">{b.profile} • {b.ext_date}</div>
                          </div>
                        ))
                      )}
                    </div>
                  ...
                  {selectedBuckets.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-2">
                      {selectedBuckets.map((b) => (
                        <span key={b.bucket_no} className="inline-flex items-center gap-1 rounded-md border border-pink-500/30 bg-pink-500/10 px-2 py-1 text-xs text-pink-300">
                          {b.bucket_no}
                          <button type="button" onClick={() => removeBucket(b.bucket_no)} className="ml-1 text-pink-400 hover:text-white">×</button>
                        </span>
                      ))}
                    </div>
                  )}
                ...
                <div>
                  <label className="mb-1 block text-xs font-medium uppercase tracking-wider text-zinc-400">Current Entry Weight (kg)</label>
                  <input type="text" value={totalWeight} readOnly className="w-full rounded-lg border border-dashed border-pink-500/30 bg-pink-500/5 px-3 py-3 text-sm font-bold text-pink-400" />
                </div>

                <div className="mt-6 flex gap-3">
                  <button type="submit" className={glassBtnPrimary}>Add Entry</button>
                  <button type="button" onClick={closeModal} className={glassBtnSecondary}>Cancel</button>
                </div>
              </form>
            </div>
          </div>
      ...
      {showLoader && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/90 backdrop-blur-xl">
          <div className="text-center">
            ...
            <h2 className="text-xl font-bold text-white">Processing Batch...</h2>
          </div>
        </div>
      )}
      ...
      {showSuccess && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/90 backdrop-blur-xl">
          <div className="rounded-3xl border border-white/10 bg-white/10 p-10 text-center shadow-2xl backdrop-blur-2xl">
            ...
            <p className="mt-2 text-sm text-zinc-400">Aging Batch Data Recorded</p>
          </div>
        </div>
      )}
    </div>
